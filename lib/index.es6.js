exports.app = function () {
      ...
      config.dotfiles.forEach(function (dotfile) {
        if(files.indexOf(dotfile.name) != -1) { // if this file is in the directory
          if(dotfile.platform === undefined || dotfile.platform === 'any' || dotfile.platform === platform) {
            dotfile.target = dotfile.target.replace(/~/, process.env.HOME)  // expand ~ to home directory
  ...
  function installVerify(dotfile) {
    ...
    try {
      let stats = fs.statSync(dotfile.target) // synchronously, since we need to read consecutively
