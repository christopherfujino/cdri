...
exports.app = function () {
  ...
  var stdin = process.openStdin(); // make global
  ...
  function init() {
    ...
    fs.readFile('config.json', 'utf8', function (err, data) {
      if (err) {} else {
        console.log(clearScreen + 'config.json file already exists!' + 'Would you like to overwrite?');
        stdin.on('data', function (chunk) {
          console.log(typeof chunk === 'undefined' ? 'undefined' : _typeof(chunk));
          chunk = ('' + chunk).toLowerCase();
          if (chunk === 'y' || chunk === 'yes') {} else if (chunk === 'n' || chunk === 'no') {
  ...
  function configValidator(config, successCallback) {
    var isSuccessful = true; // should we ever set this false?
    ...
    fs.readdir(process.cwd(), function (err, files) {
      if (err) throw err;
      config.dotfiles.forEach(dotfileValidator);
      if (isSuccessful) successCallback();
    }); // reads the contents of a directory
  }

  function dotfileValidator(dotfile) {
